...
SELECT COUNT(*) AS screened, CAST(SUM(ws.giving_capacity - ISNULL(ds.total_given,0)) AS DECIMAL(14,0)) AS unrealized, CAST(ROUND(AVG(CASE WHEN ws.giving_capacity>0 THEN ISNULL(ds.total_given,0)/ws.giving_capacity*100 ELSE 0 END),1) AS DECIMAL(5,1)) AS avg_util, SUM(CASE WHEN ws.giving_capacity>0 AND ISNULL(ds.total_given,0) < ws.giving_capacity*0.1 THEN 1 ELSE 0 END) AS below_10pct FROM serving.wealth_screening ws LEFT JOIN serving.donor_summary ds ON ds.person_id=ws.person_id WHERE ws.display_name<>'Unknown'
→ **stat_grid**: stats=[{label:"Screened Contacts",value:screened},{label:"Unrealized Capacity",value:unrealized,unit:"$"},{label:"Avg Utilization",value:avg_util+"%"},{label:"Below 10% Capacity",value:below_10pct}]

**Query 2 — Distribution** (for bar_chart):
SELECT CASE WHEN ISNULL(ds.total_given,0)/NULLIF(ws.giving_capacity,0)*100 < 1 THEN '0-1%' WHEN ISNULL(ds.total_given,0)/NULLIF(ws.giving_capacity,0)*100 < 5 THEN '1-5%' WHEN ISNULL(ds.total_given,0)/NULLIF(ws.giving_capacity,0)*100 < 10 THEN '5-10%' WHEN ISNULL(ds.total_given,0)/NULLIF(ws.giving_capacity,0)*100 < 25 THEN '10-25%' WHEN ISNULL(ds.total_given,0)/NULLIF(ws.giving_capacity,0)*100 < 50 THEN '25-50%' ELSE '50%+' END AS [Utilization Range], COUNT(*) AS [Contacts] FROM serving.wealth_screening ws LEFT JOIN serving.donor_summary ds ON ds.person_id=ws.person_id WHERE ws.display_name<>'Unknown' AND ws.giving_capacity>0 GROUP BY CASE WHEN ISNULL(ds.total_given,0)/NULLIF(ws.giving_capacity,0)*100 < 1 THEN '0-1%' WHEN ISNULL(ds.total_given,0)/NULLIF(ws.giving_capacity,0)*100 < 5 THEN '1-5%' WHEN ISNULL(ds.total_given,0)/NULLIF(ws.giving_capacity,0)*100 < 10 THEN '5-10%' WHEN ISNULL(ds.total_given,0)/NULLIF(ws.giving_capacity,0)*100 < 25 THEN '10-25%' WHEN ISNULL(ds.total_given,0)/NULLIF(ws.giving_capacity,0)*100 < 50 THEN '25-50%' ELSE '50%+' END ORDER BY MIN(ISNULL(ds.total_given,0)/NULLIF(ws.giving_capacity,0)*100)
→ **bar_chart**: categoryKey='Utilization Range', valueKeys=['Contacts'], title="Giving-to-Capacity Distribution"

**Query 3 — Full table** sorted by gap:
SELECT ws.display_name AS [Contact], ws.capacity_label AS [Tier], CAST(ISNULL(ds.total_given,0) AS DECIMAL(12,2)) AS [Lifetime Giving], CAST(ws.giving_capacity AS DECIMAL(12,2)) AS [Capacity], CAST(ws.giving_capacity - ISNULL(ds.total_given,0) AS DECIMAL(12,2)) AS [Gap], CAST(ROUND(ISNULL(ds.total_given,0)/NULLIF(ws.giving_capacity,0)*100,1) AS DECIMAL(5,1)) AS [% Utilized] FROM serving.wealth_screening ws LEFT JOIN serving.donor_summary ds ON ds.person_id=ws.person_id WHERE ws.display_name<>'Unknown' ORDER BY [Gap] DESC
→ **table**: title="Wealth-Screened Contacts — Capacity vs. Lifetime Giving (Sorted by Gap)"

**Widget 4 — text**: Strategic analysis — who are the biggest opportunities by name, which tiers have the most unrealized capacity, how many screened contacts have NEVER donated (gap = 100% of capacity), specific outreach recommendations.
