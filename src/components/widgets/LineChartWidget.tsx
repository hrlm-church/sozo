...
export function LineChartWidget({ widget }: { widget: Widget }) {
  ...
  return (
    <ChartContainer config={chartConfig} style={{ height: "100%" }}>
      <LineChart data={data} margin={{ top: 8, right: 8, bottom: showLegend ? 4 : 0, left: 0 }}>
        <defs>
          {valueKeys.map((key) => (
            <linearGradient key={key} id={`line-shadow-${key}`} x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={chartConfig[key].color} stopOpacity={0.15} />
              <stop offset="100%" stopColor={chartConfig[key].color} stopOpacity={0} />
            </linearGradient>
          ))}
        </defs>
        ...
        {valueKeys.map((key) => (
          <Line
            ...
            activeDot={{ r: 5, fill: chartConfig[key].color, stroke: "#fff", strokeWidth: 2 }}
