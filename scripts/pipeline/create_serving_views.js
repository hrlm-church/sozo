...
const VIEWS = [
  // ── 1. PERSON 360 ────────────────────────────────────────────────────
  {
    ...
    SELECT
      ...
      COALESCE(c.email_primary, (SELECT TOP 1 c2.email_primary FROM silver.identity_map im2 JOIN silver.contact c2 ON c2.contact_id = im2.contact_id WHERE im2.master_id = im.master_id AND c2.email_primary IS NOT NULL AND c2.email_primary <> '' ORDER BY c2.contact_id)) AS email,
      COALESCE(c.phone_primary, (SELECT TOP 1 c2.phone_primary FROM silver.identity_map im2 JOIN silver.contact c2 ON c2.contact_id = im2.contact_id WHERE im2.master_id = im.master_id AND c2.phone_primary IS NOT NULL AND c2.phone_primary <> '' ORDER BY c2.contact_id)) AS phone,
  ...
  // Treats literal "NULL" strings as SQL NULL for name construction.
  {
    ...
    WITH raw_donations AS (
      SELECT
        ...
        COALESCE(c.email_primary, (SELECT TOP 1 c2.email_primary FROM silver.identity_map im2 JOIN silver.contact c2 ON c2.contact_id = im2.contact_id WHERE im2.master_id = COALESCE(im.master_id, c.contact_id) AND c2.email_primary IS NOT NULL AND c2.email_primary <> '' ORDER BY c2.contact_id)) AS email,
        d.amount, d.currency,
        d.donated_at,
  ...
  // ── 9. SUBSCRIPTION DETAIL ─────────────────────────────────────────
  {
    ...
    SELECT
      ...
      COALESCE(pc.email_primary, (SELECT TOP 1 c2.email_primary FROM silver.identity_map im2 JOIN silver.contact c2 ON c2.contact_id = im2.contact_id WHERE im2.master_id = im_s.master_id AND c2.email_primary IS NOT NULL AND c2.email_primary <> '' ORDER BY c2.contact_id)) AS email,
      ss.product_name,
