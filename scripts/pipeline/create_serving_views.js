...
const VIEWS = [
  // ── 1. PERSON 360 ────────────────────────────────────────────────────
  {
    ...
    SELECT
      ...
      c.first_name, c.last_name, c.email_primary AS email, c.phone_primary AS phone,
  ...
  // ── 3. DONATION DETAIL ──────────────────────────────────────────────
  // Uses LEFT JOIN to identity_map so kindful/keap_import donations (not in identity_map) are included
  {
    ...
    SELECT
      ...
      COALESCE(c.first_name + ' ' + c.last_name, c.first_name, c.last_name, 'Unknown') AS display_name,
      c.first_name, c.last_name, c.email_primary AS email,
      d.amount, d.currency,
      d.donated_at,
  ...
  // ── 5. DONOR MONTHLY ───────────────────────────────────────────────
  {
    ...
    FROM serving.donation_detail
    WHERE amount > 0 AND donated_at IS NOT NULL
